...
from air.setups.signal import TradeExitSignal
from air.setups.simple_setups import *
from air.utils import ListFileReader
from air.backtest import BackTesterDatabase, BackTesterCandles
from air.filters.simple_filters import LambdaSelectFilter
...
pdb.set_trace()
legit_exit_signals = [TradeExitSignal.create(ts.the_date + datetime.timedelta(minutes=480), ts.instrument, ts.strategy_ref, ts.direction) for i,ts in signals.iterrows()]
...
#now backtest
all_dates = [s.the_date for i,s in signals.iterrows()]
