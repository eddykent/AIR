...
from air.indicators.indicator import Typical
import air.charting.chart_viewer as chv

from air.setups.setup_tools import DivTool
...
pdb.set_trace()
div_tool = DivTool()
bullish, bearish = div_tool.markup((price_action_result[:,:,0],momentum_result[:,:,0]))
...
def show_chart(instrument_index, snap_index=-1):
	...
	result_bias = bullish[instrument_index].astype(int) - bearish[instrument_index].astype(int)
	#pdb.set_trace()
