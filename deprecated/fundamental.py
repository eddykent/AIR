...
##this file is currently frozen - it may become deprecated in future as it is not clear if the
##thinking behind it is correct
...
class NewsIndicator(Indicator):
	...
	def draw_snapshot(self,np_candles : np.array ,instrument_index: int,snapshot_index : int = -1) -> chv.ChartView: 
		
		this_view = chv.ChartView() 
		candle_stream = np_candles[instrument_index]
		candle_stream_timeline = candle_stream[:,-1] #candle start date should be at bottom of each candle 
