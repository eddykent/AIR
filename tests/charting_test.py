...
for i,e in enumerate(pattern_result):
	if e != 0:		
		(x,y,arrow_x,arrow_y,these_candles,these_indexs) = candle_pattern.draw_individual(i,candles) #maybe better way of doing this than sending hugh tuples one by one
		highlight_box_trace = chart.Scatter(
			x=x,
			y=y,
			fill='toself',
			name='Engulfing',
			fillcolor='rgba(255,0,0,0.3)' if e < 0 else 'rgba(0,255,0,0.3)',
			mode='lines',
