...
class RollingExtremeStop(StopTool):
	...
	def __init__(self, risk_reward_ratio=1.5,period=5,**kwargs):
		self.period = period
		super().__init__(**kwargs)
...
class RollingPipBuffStops(StopTool):
	
	pip_stop = None
	roll_extreme = None 
	
	def __init__(self,risk_reward_ratio=1.5,period=10,tp_buff=5,sl_buff=5):	
		self.pip_stop = PipStop(tp_buff,sl_buff)
		self.roll_extreme = RollingExtremeStop(risk_reward_ratio,period)
	
	def get_stops(self,trade_signalling_data):
		(bull_tp_p, bull_sl_p), (bear_tp_p, bear_sl_p) = self.pip_stop.get_stops(trade_signalling_data)
		(bull_tp_r, bull_sl_r), (bear_tp_r, bear_sl_r) = self.roll_extreme.get_stops(trade_signalling_data)
		return (bull_tp_p + bull_tp_r, bull_sl_p + bull_sl_r), (bear_tp_p + bear_tp_r, bear_sl_p + bear_sl_r)
