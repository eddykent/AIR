...
ich_lag = ValueLagTool(26)#standard
def ichimoku_bullish(res,npc):
	clouda = ich_lag.markup(res[:,:,2])
	cloudb = ich_lag.markup(res[:,:,3])
	
	cloud = (clouda > cloudb)
	base_conv = (res[:,:,0] > res[:,:,1])
	price_place = (npc[:,:,csf.close] > clouda)
	
	return cloud & base_conv & price_place
	
def ichimoku_bearish(res,npc):
	clouda = ich_lag.markup(res[:,:,2])
	cloudb = ich_lag.markup(res[:,:,3])
...
def mass_index_func(vv=26):
	mi_lag = ValueLagTool(1)
	def inner_mass_index_func(res,npc):
		current = res[:,:,0]
...
def fg_bullish(res,npc):
	(res[:,:,0] < 0) & (res[:,:,1] < 0)

def fg_bearish(res,npc):
	(res[:,:,0] > 0) & (res[:,:,1] > 0)

def cross_up_below_zero(res,npc):
	bullish, bearish = CrossTool.markup(res[:,:,0] - res[:,:,1])
	return bullish & (res[:,:,0] < 0)
	
	
def cross_down_above_zero(res,npc):
	bullish, bearish = CrossTool.markup(res[:,:,0] - res[:,:,1])
	return bearish & (res[:,:,0] > 0)
...
def full_set(trade_signalling_data):
	...
	return [
		...
		TriggerBlock(FourierGradient(), fg_bullish, fg_bearish , 'gradient & price diff positive'), 
...
#Shapes, Ichimoku, Trends, MassIndex, CCI, WilliamsPercentRange
#awesome?, accelerator

def high_rare1(trade_signalling_data):	
	return [
		#chart patterns are quite rare -unravel?
		TriggerBlock(PivotPoints(trade_signalling_data.timeline), default_bullish, default_bearish),
		SetupBlock(Harmony(),trade_signalling_data), 
		SetupBlock(Triangles(),trade_signalling_data),
		SetupBlock(Shapes(),trade_signalling_data),
		...
		#rare instances of oscillators - divergences 
		TriggerBlock(RSI(21),divergences_bullish, divergences_bearish,'divergence'), 
		TriggerBlock(RSI(21),hidden_divergences_bullish, hidden_divergences_bearish,'hidden divergence'),
		
		#other rare oscillators 
		TriggerBlock(RSI(21), oscillator_reversal_bullish(0.2), oscillator_reversal_bearish(0.8), '0.2 and 0.8'),
		TriggerBlock(MACD(), cross_up_below_zero, cross_down_above_zero, 'only when cross happened & below 0'), 
		
		
		TriggerBlock(IchimokuCloud(),ichimoku_bullish,ichimoku_bearish,'standard ichimoku'),
		TriggerBlock(Aroon(25), lambda res, npc: res[:,:,1] > res[:,:,2], lambda res, npc : res[:,:,2] > res[:,:,1], 'up bigger than down'),
		
		
		TriggerBlock(BollingerBands(20,2.5), lambda res, npc : npc[:,:,csf.close] < res[:,:,2], lambda res, npc : npc[:,:,csf.close] > res[:,:,1] ,'standard close above/below upper/lower bands'),
		TriggerBlock(KeltnerChannel(20,2.5,14), lambda res, npc : npc[:,:,csf.high] > res[:,:,1], lambda res, npc : npc[:,:,csf.low] > res[:,:,2]),
		TriggerBlock(FourierGradient(), fg_bullish, fg_bearish , 'gradient & price diff positive'), 
...
def mostly_set(trade_signalling_data):
	...
	return [ 
		...
		#TriggerBlock(RSI(5), oscillator_reversal_bullish(0.15), oscillator_reversal_bearish(0.85), '0.15 and 0.85'),
		TriggerBlock(CCI(), oscillator_reversal_bullish(0.2), oscillator_reversal_bearish(0.8), '0.2 and 0.8'),
		TriggerBlock(MACD(), cross_up_below_zero, cross_down_above_zero,'included everything after cross, but macd below 0 for buy'),
		TriggerBlock(MoneyFlowIndex(), oscillator_reversal_bullish(0.2), oscillator_reversal_bearish(0.8), '0.2 and 0.8'),		
		#TriggerBlock(Accelerator(), lambda res, npc : res[:,:,0] > 0, lambda res, npc : res[:,:,0] < 0,'zero line based'), 
		#Awesome?
		TriggerBlock(RVI(), lambda res,npc : res[:,:,0] > res[:,:,1], lambda res,npc : res[:,:,0] < res[:,:,1], 'signal cross'),
		...
		#TriggerBlock(match_pattern, lambda res, npc : res[:,:,0] > 0, lambda res, npc : res[:,:,0] < 0, 'find matchings'),
		TriggerBlock(ADX(), lambda res, npc : res[:,:,0] > 0.25, lambda res, npc : res[:,:,0] > 0.25, 'check trending only'),
		TriggerBlock(ADX(), lambda res, npc : res[:,:,1] > 0.75, lambda res, npc : res[:,:,2] > 0.75, 'check positives and negatives'),
		TriggerBlock(ChoppinessIndex(), lambda res, npc : res[:,:,0] < res[:,:,1], lambda res, npc : res[:,:,0] < res[:,:,1]),
		TriggerBlock(IchimokuCloud(),ichimoku_bullish,ichimoku_bearish,'standard ichimoku'),
		TriggerBlock(ParabolicSAR(), lambda res, npc : ~np.isnan(res[:,:,0]), lambda res, npc : ~np.isnan(res[:,:,1]), 'check for stars'),
		TriggerBlock(Aroon(25), lambda res, npc: res[:,:,1] > res[:,:,2], lambda res, npc : res[:,:,2] > res[:,:,1], 'up bigger than down'),
