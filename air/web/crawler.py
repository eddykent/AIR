...
#from selenium.webdriver import ChromeOptions #done on init
...
#wrapper class for crawler that is powered by xpath to crawl a webpage and find elements. Always start from the root node
#hopefully dicts are easier to understand? 
class XPathNavigator(Crawler):
	...
	#obv dont use scientific notation >:-/
	def type_number_on(self,element,numeric,sf=3):
		...
		n = len(current_val)
		for i in range(n):
			element.send_keys(Keys.BACKSPACE) #wipe current input
		...
		if new_val0 != int(new_val0) or len(number_pieces) > 1:
			#we have something fractional. 
			element.send_keys('.') #try sending it a decimal point
			if self.get_attribute(element,'value').endswith('.'):#does it take decimals?
