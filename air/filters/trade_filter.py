...
class FlatIndicatorFilter(TimelineTradeFilter):  ##base this on an indicator so any indicator can be used as a filter? 
	...
	timeline = None   ##should use signalling data instead
	instruments = None
	np_candles = None
	candle_streams = None 
	candle_length = None
...
class PartialIndicatorFilter:
	...
	def _get_n_candles_in_partial(self,trade_signal,min_length=15):
		ti = self.signalling_data.closest_time_index(trade_signal.the_date)
...
#other simple programatic filters 
...
##use for when we KNOW there are bad trades and we dont want to repeat them 
class AntiSignalFilter(TimelineTradeFilter):
	
	anti_signals = None  #df
	
	def __init__(self, anti_signals):
		self.anti_signals = anti_signals
		
	def filter(self,signals):  #left join null (return everything in signals and not in anti signals)
		signals_all = signals.merge(self.anti_signals, on=['instrument','direction','the_date'], how='left',indicator=True)
		left_signals = signals_all[signals_all['_merge'] == 'left_only'].copy()
		return_signals = pd.DataFrame([])
		#pdb.set_trace()
		return_signals['signal_id'] = left_signals['signal_id_x']
		return_signals['instrument'] = left_signals['instrument']
		return_signals['direction'] = left_signals['direction']
		return_signals['the_date'] = left_signals['the_date']
		return_signals['strategy_ref'] = left_signals['strategy_ref_x']
		return_signals['entry'] = left_signals['entry_x']
		return_signals['entry_cut'] = left_signals['entry_cut_x']
		return_signals['entry_expire'] = left_signals['entry_expire_x']
		return_signals['take_profit_distance'] = left_signals['take_profit_distance_x']
		return_signals['stop_loss_distance'] = left_signals['stop_loss_distance_x']
		return_signals['length'] = left_signals['length_x']
