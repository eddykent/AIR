...
class NewsFilter(TimelineTradeFilter): #fixme
...
class AIInvokeFilter(TimelineTradeFilter):
	
	model = None 
	invoker = None #function to call on Y data for yes or no 
	
	def __init__(self,model, invoke_fn):
		self.model = model
		self.invoker = invoke_fn
	
	def filter(self,trades):
		results = self.model.invole(trades)
		
		#either, accept [1,0] or reject [0,1] 
		indexer = self.invoker(results)
		return trades[indexer]
		#other lenient way:
		#return trades[~(results[:,1] == 1)]
