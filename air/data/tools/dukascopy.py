...
WHERE bid_volume > %(volume_threshold)s AND ask_volume > %(volume_threshold)s;
	...
	def _check_db_merge(self, instrument, errors):
		...
		new_errors  = {}
		new_errors['bid'] = [dt for dt in errors.get('bid',[]) if dt not in ok_slots]
		new_errors['ask'] = [dt for dt in errors.get('ask',[]) if dt not in ok_slots]
		new_errors['bid_volume'] = [dt for dt in errors.get('bid_volume',[]) if dt not in ok_volumes]
		new_errors['ask_volume'] = [dt for dt in errors.get('ask_volume',[]) if dt not in ok_volumes]
		
		if not len(new_errors['bid']):
			del new_errors['bid']
		if not len(new_errors['ask']):
			del new_errors['ask']
		if not len(new_errors['bid_volume']):
			del new_errors['bid_volume']
		if not len(new_errors['ask_volume']):
			del new_errors['ask_volume']
