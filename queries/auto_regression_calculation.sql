...
correlations AS (
	...
	REGR_INTERCEPT(candle1_percent_change, candle2_lag_percent_change) OVER w AS intercept, --this actually causes problems. but we are working with percent change so should be 0 
	lag_index 
	FROM all_changes
	WINDOW w AS (
...
collected_percent_changes AS (
	...
	SUM(CASE WHEN ABS(correlation) > 0.27 THEN (slope*candle2_leadlag) ELSE 0 END) AS next_percent_changes,
	SUM(CASE WHEN ABS(correlation) > 0.27 THEN (slope*candle2_leadlag)*(slope*candle2_leadlag) ELSE 0 END) AS next_percent_changes_squared,
	SUM(CASE WHEN ABS(correlation) > 0.27 THEN 1 ELSE 0 END) AS n_percent_changes
...
average_percent_changes AS (
	...
	CASE WHEN n_percent_changes = 0 THEN 0 ELSE 
...
--SELECT * FROM exchange_valASue_tick evt  WHERE from_currency = 'USD' AND to_currency = 'JPY' AND the_date > '2022-01-20 12:00:00.000'
