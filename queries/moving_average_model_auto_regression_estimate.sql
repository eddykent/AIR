...
solid_averages AS (
	...
	WINDOW w AS (PARTITION BY from_currency, to_currency ORDER BY the_date ASC ROWS BETWEEN (%(sma_window)s::INT - 1) PRECEDING AND CURRENT ROW)
),
...
total_errors AS (
	...
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN (slope*next_prev_epsilon) ELSE 0 END) AS total_epsilons,
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN 1 ELSE 0 END) AS n 
