...
WITH candle_indexs AS (
	...
	AND the_date < (DATE '20 Jan 2022' + INTERVAL '8 hours') --what about IF we want TO START ON 45 mins?
	AND the_date >= (DATE '20 Jan 2022' - INTERVAL '100 days' - INTERVAL '20 days' + INTERVAL '8 hours') --600 = 400 + 200 (days_back + normalisation_window)
),
...
look_backs AS (
	SELECT generate_series(1,6,1) AS lag_index
),
all_changes AS (
	...
	look_backs.lag_index,
	--lag or lead?
	LAG(candle1.percent_change,look_backs.lag_index) OVER (PARTITION BY candle1.from_currency, candle1.to_currency, candle2.from_currency, candle2.to_currency, look_backs.lag_index ORDER BY candle1.the_date ASC ) AS candle1_lag_percent_change,
	LAG(candle2.percent_change,look_backs.lag_index) OVER (PARTITION BY candle2.from_currency, candle2.to_currency, candle1.from_currency, candle1.to_currency, look_backs.lag_index ORDER BY candle2.the_date ASC ) AS candle2_lag_percent_change
...
correlations AS (
	...
	WINDOW w AS (
		...
		ORDER BY the_date ASC ROWS BETWEEN 29 PRECEDING AND CURRENT ROW --correlate LAST 20 entries
	)
--	GROUP BY candle1_from_currency, candle1_to_currency, candle2_from_currency, candle2_to_currency
)
SELECT * FROM correlations WHERE correlation = 1
,
candle2_lead_lag AS ( --this IS confusing, but it needs TO have the current SET OF candles TO predict the NEXT one NOT the previous ones 
	SELECT *,
	CASE WHEN lag_index = 1 THEN candle2_percent_change ELSE LEAD(candle2_lag_percent_change,1) OVER w END AS candle2_leadlag
	FROM correlations
	WINDOW w AS (
		PARTITION BY candle1_from_currency, candle1_to_currency, candle2_from_currency, candle2_to_currency 
		ORDER BY the_date ASC ROWS BETWEEN 29 PRECEDING AND CURRENT ROW --correlate LAST 20 entries
...
predicted_percent_changes AS (
	...
	lag_index,
	AVG(CASE WHEN correlation > 0.3 THEN slope*candle2_leadlag + intercept ELSE 0 END) AS candle1_next_percent_change
	FROM candle2_lead_lag 
	GROUP BY the_date, candle1_from_currency, candle1_to_currency, lag_index--, candle2_lead_percent_change
),
total_percent_changes AS (
	SELECT the_date, 
	from_currency,
	to_currency, 
	SUM(candle1_next_percent_change) AS predicted_percent_change
...
--json_object_agg(
--	from_currency || '/' || to_currency, predicted_percent_change
...
FROM total_percent_changes
WHERE the_date = '2022-01-20 04:00:00.000'
