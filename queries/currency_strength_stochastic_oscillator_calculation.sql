...
DROP TABLE IF EXISTS tmp_all_stochastic_oscillators; 
...
calc_d AS (
	...
	the_date,
	k AS stochastic_k,
	AVG(k) OVER w AS stochastic_d
	FROM calc_k
	WINDOW w AS (PARTITION BY from_currency, to_currency ORDER BY the_date ASC ROWS BETWEEN (3::INT-1) PRECEDING AND CURRENT ROW)
),
scaled_kd AS (
	SELECT from_currency, to_currency,
	(2.0*stochastic_k) - 1.0 AS stochastic_k,
	(2.0*stochastic_d) - 1.0 AS stochastic_d, --SCALE TO [-1, 1]
	the_date
	FROM calc_d
)
SELECT from_currency, to_currency, the_date, stochastic_k, stochastic_d
INTO TEMPORARY TABLE tmp_all_stochastic_oscillators FROM scaled_kd;

SELECT cs.currency, so.stochastic_d AS movement, so.the_date 
INTO TEMPORARY TABLE tmp_friends
FROM tmp_all_stochastic_oscillators so
JOIN tmp_currencies cs ON so.from_currency = cs.currency;

SELECT cs.currency, so.stochastic_d AS movement, so.the_date 
INTO TEMPORARY TABLE tmp_enemies
FROM tmp_all_stochastic_oscillators so
JOIN tmp_currencies cs ON so.to_currency = cs.currency;
...
WITH all_dates_pairs AS (
	SELECT DISTINCT the_date, currency FROM tmp_all_stochastic_oscillators, tmp_currencies ORDER BY the_date
),
all_movements AS (
	SELECT c.currency, 
	...
	SELECT c.currency,
	c.the_date AS the_date,
	-e.movement AS movement
	FROM all_dates_pairs c
	JOIN tmp_enemies e ON c.currency = e.currency AND c.the_date = e.the_date
),
movements AS (
	SELECT currency, the_date, SUM(movement) AS movement 
	FROM all_movements 
