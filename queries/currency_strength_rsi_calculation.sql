...
WITH candle_indexs AS (
	...
	AND the_date < (DATE '14 Feb 2022' + INTERVAL '16  hours')
	AND the_date >= (DATE '14 Feb 2022' - INTERVAL '200 days' + INTERVAL '16  hours') 
	--AND from_currency = 'AUD' AND to_currency = 'CAD' AND the_date > '26 Apr 2020 22:00'::TIMESTAMP 
	--AND the_date < '26 Apr 2020 22:15'::TIMESTAMP 
),
...
SELECT from_currency, to_currency, close_price, rate_of_change, time_index, the_date INTO TEMPORARY TABLE tmp_all_rates_of_change FROM rates_of_change;
...
--AND currency = 'EUR'
GROUP BY the_date
HAVING SUM(1) = (SELECT ARRAY_LENGTH(ARRAY['AUD','CAD','CHF','EUR','GBP','NZD','JPY','USD'],1))
