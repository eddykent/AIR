...
WITH candle_indexs AS (
	...
	AND the_date >= (DATE %(the_date)s - INTERVAL '%(hour)s days' + INTERVAL '%(hour)s hours') --600 = 400 + 200 (days_back + normalisation_window)
),
...
collected_strength_differences AS (
	...
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN (slope*strength2_leadlag+intercept) ELSE 0 END) AS sum_next_strength_differences,
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN (slope*strength2_leadlag+intercept)*(slope*strength2_leadlag+intercept) ELSE 0 END) AS sum_squared_next_strength_differences,
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN 1 ELSE 0 END) AS n_next_strength_differences
