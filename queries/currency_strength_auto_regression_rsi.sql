--query for regressing from all correlations between each currency pair that we are interested in
--and forecasting what the next currency strength will be. 
...
lagged_strengths AS (
	...
	strength,
	lag_index,
	LAG(strength,lag_index) OVER (PARTITION BY currency, lag_index ORDER BY the_date) AS lag_strength
	FROM tmp_all_strengths, look_backs
),
all_changes AS (
	SELECT strength1.the_date,
	strength1.strength AS strength1_strength,
	strength2.strength AS strength2_strength,
	strength1.currency AS strength1_currency,
	strength2.currency AS strength2_currency,
	strength2.lag_strength AS strength2_lag_strength,
	strength2.lag_index
...
correlations AS (
	...
	strength1_currency, 
	strength2_currency,
	strength2_lag_strength,
	CORR(strength1_strength,strength2_lag_strength) OVER w AS correlation, --correlates with the next difference
	REGR_SLOPE(strength1_strength, strength2_lag_strength) OVER w AS slope,
	REGR_INTERCEPT(strength1_strength, strength2_lag_strength) OVER w AS intercept, 
	lag_index 
	FROM all_changes
	WINDOW w AS (
		PARTITION BY strength1_currency, strength2_currency, lag_index
		ORDER BY the_date ASC ROWS BETWEEN %(correlation_window)s::INT PRECEDING AND CURRENT ROW --correlate last 8 entries - works well with low correlations on 12 Jan 2022 AT 12pm.. so does high correl! 
	)
...
strength2_lead_lag AS ( --this IS confusing, but it needs TO have the current SET OF candles TO predict the NEXT one NOT the previous ones 
	SELECT *,
	CASE WHEN lag_index = 1 THEN strength2_strength ELSE LAG(strength2_lag_strength,1) OVER w END AS strength2_leadlag
	FROM correlations
	WINDOW w AS (
		PARTITION BY strength1_currency, strength2_currency, lag_index
...
,
collected_strengths AS (
	SELECT 
	the_date,
	strength1_currency AS currency,
	strength1_strength AS strength,
	--lag_index, --intercept causes issues
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN (slope*strength2_leadlag+intercept) ELSE 0 END) AS sum_next_strengths,
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN (slope*strength2_leadlag+intercept)*(slope*strength2_leadlag+intercept) ELSE 0 END) AS sum_squared_next_strengths,
	SUM(CASE WHEN ABS(correlation) > %(correlation_threshold)s::DOUBLE PRECISION THEN 1 ELSE 0 END) AS n_next_strengths
	FROM strength2_lead_lag 
	GROUP BY the_date, currency, strength1_strength
),
average_strengths AS (
	SELECT the_date, 
	currency,
	strength,
	CASE WHEN n_next_strengths = 0 THEN strength ELSE sum_next_strengths / n_next_strengths END AS average_next_strength,
	CASE WHEN n_next_strengths = 0 THEN 0 ELSE 
		(sum_squared_next_strengths / n_next_strengths) - ((sum_next_strengths / n_next_strengths)*(sum_next_strengths / n_next_strengths))
	END AS next_strength_variance,
	n_next_strengths 
	FROM collected_strengths 
),
next_strengths AS (
	SELECT 
	the_date,
	currency, 
	next_strength_variance,
	n_next_strengths,
	strength,
	average_next_strength AS next_strength
	FROM average_strengths
)
