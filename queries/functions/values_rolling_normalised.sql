...
DROP FUNCTION IF EXISTS trading.values_rolling_normalised(_values_tmp TEXT,  _period INTEGER);
CREATE OR REPLACE FUNCTION trading.values_rolling_normalised(_values_tmp TEXT,  _period INTEGER)
RETURNS TABLE (
	row_index INTEGER, 
	full_name TEXT,
	the_date TIMESTAMP,
...
COMMENT ON FUNCTION trading.values_rolling_normalised(TEXT, INTEGER) IS 'From a set of values, return their normed value.';
