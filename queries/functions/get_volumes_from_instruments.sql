...
--from currencies 
DROP FUNCTION IF EXISTS trading.get_volumes_from_instruments(TEXT[], timestamp, int, int, int);
CREATE OR REPLACE FUNCTION trading.get_volumes_from_instruments(_instruments TEXT[], _this_date TIMESTAMP, _days_back INTEGER, _chart_resolution INTEGER DEFAULT 15, _candle_offset INTEGER DEFAULT 0)
RETURNS TABLE (
	row_index INTEGER,	
	from_currency TEXT,
	to_currency TEXT,
...
BEGIN
	...
	DROP TABLE IF EXISTS _get_volumes_from_instruments;

	--build volumes of our chosen chart size 
	CREATE TEMPORARY TABLE _get_volumes_from_instruments
	ON COMMIT DROP  
	AS (
	...
	CREATE INDEX _get_volumes_from_instruments_row_index_idx ON _get_volumes_from_instruments USING btree(row_index);
	CREATE INDEX _get_volumes_from_instruments_full_name_idx ON _get_volumes_from_instruments USING btree(full_name);
	CREATE INDEX _get_volumes_from_instruments_the_date_idx ON _get_volumes_from_instruments USING btree(the_date); --check

	RETURN QUERY SELECT * FROM _get_volumes_from_instruments; 

END
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION trading.get_volumes_from_instruments(TEXT[], timestamp, int, int, int) IS 'From a set of currencies, and a timestamp, get the associated forex volumes.';
