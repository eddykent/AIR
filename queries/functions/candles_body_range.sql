...
DROP FUNCTION IF EXISTS candles_body_range(_candles_tmp TEXT);
CREATE OR REPLACE FUNCTION candles_body_range(_candles_tmp TEXT)
RETURNS TABLE (
	row_index INTEGER, 
	full_name TEXT,
	the_date TIMESTAMP,
...
COMMENT ON FUNCTION candles_body_range(TEXT) IS 'From a set of candles, get their mean price';
