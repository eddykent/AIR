...
--DEPENDS ON EMA
DROP FUNCTION IF EXISTS trading.values_instrument_ranking(_values_tmp TEXT);
CREATE OR REPLACE FUNCTION trading.values_instrument_ranking(_values_tmp TEXT)
RETURNS TABLE (
	row_index INTEGER, 
	full_name TEXT,
	the_date TIMESTAMP,
...
BEGIN 
	...
	AS (
		...
		vir.full_name,
		vir.the_date, 
		(ROW_NUMBER() OVER (PARTITION BY vir.the_date ORDER BY vir.value DESC))::INTEGER AS ranking
...
COMMENT ON FUNCTION trading.values_instrument_ranking(TEXT) IS 'For every timestep, rank the instruments by their value';
