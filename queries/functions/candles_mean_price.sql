...
DROP FUNCTION IF EXISTS trading.candles_mean_price(_candles_tmp TEXT);
CREATE OR REPLACE FUNCTION trading.candles_mean_price(_candles_tmp TEXT)
RETURNS TABLE (
	row_index INTEGER, 
	full_name TEXT,
	the_date TIMESTAMP,
...
COMMENT ON FUNCTION trading.candles_mean_price(TEXT) IS 'From a set of candles, get their mean price';
