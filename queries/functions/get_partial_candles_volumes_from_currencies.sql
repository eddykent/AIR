...
--DROP FUNCTION IF EXISTS trading.get_partial_candles_volumes_from_currencies(TEXT[], TIMESTAMP[], int, int);
DROP FUNCTION IF EXISTS trading.get_partial_candles_volumes_from_currencies(TEXT[], TIMESTAMP[], int, int, bool);
CREATE OR REPLACE FUNCTION trading.get_partial_candles_volumes_from_currencies(_currencies TEXT[], _trade_times TIMESTAMP[], _chart_resolution INTEGER DEFAULT 15, _candle_offset INTEGER DEFAULT 0, _ask_candles BOOLEAN DEFAULT FALSE)
RETURNS TABLE (
	row_index INTEGER,	
	from_currency TEXT,
	to_currency TEXT,
...
BEGIN 
	...
	DROP TABLE IF EXISTS _get_partial_candles_volumes_from_currencies;
	CREATE TEMPORARY TABLE _get_partial_candles_volumes_from_currencies
	ON COMMIT DROP 
	AS (
		WITH the_range AS (
			SELECT 
	...
	CREATE INDEX _get_partial_candles_volumes_from_currencies_row_index_idx ON _get_partial_candles_volumes_from_currencies USING btree(row_index);
	CREATE INDEX _get_partial_candles_volumes_from_currencies_full_name_idx ON _get_partial_candles_volumes_from_currencies USING btree(full_name);
	CREATE INDEX _get_partial_candles_volumes_from_currencies_the_date_idx ON _get_partial_candles_volumes_from_currencies USING btree(the_date); --check

	RETURN QUERY SELECT * FROM _get_partial_candles_volumes_from_currencies; 

END
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION trading.get_partial_candles_volumes_from_currencies(TEXT[], TIMESTAMP[], int, int, bool) IS 'From a set of currencies and candle end times, get the associated forex partial candles and volumes';
