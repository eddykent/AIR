...
--from currencies 
--DROP FUNCTION IF EXISTS trading.get_candles_volumes_from_instruments(TEXT[], timestamp, int, int, int);
DROP FUNCTION IF EXISTS trading.get_candles_volumes_from_instruments(TEXT[], timestamp, int, int, int, bool);
CREATE OR REPLACE FUNCTION trading.get_candles_volumes_from_instruments(_instruments TEXT[], _this_date TIMESTAMP, _days_back INTEGER, _chart_resolution INTEGER DEFAULT 15, _candle_offset INTEGER DEFAULT 0, _ask_candles BOOLEAN DEFAULT FALSE)
RETURNS TABLE (
	row_index INTEGER,	
	from_currency TEXT,
	to_currency TEXT,
...
BEGIN	
	DROP TABLE IF EXISTS _get_candles_volumes_from_instruments;

	--build candles of our chosen chart size 
	CREATE TEMPORARY TABLE _get_candles_volumes_from_instruments
	ON COMMIT DROP  
	AS (
	...
	CREATE INDEX _get_candles_volumes_from_instruments_row_index_idx ON _get_candles_volumes_from_instruments USING btree(row_index);
	CREATE INDEX _get_candles_volumes_from_instruments_full_name_idx ON _get_candles_volumes_from_instruments USING btree(full_name);
	CREATE INDEX _get_candles_volumes_from_instruments_the_date_idx ON _get_candles_volumes_from_instruments USING btree(the_date); --check

	RETURN QUERY SELECT * FROM _get_candles_volumes_from_instruments; 

END
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION trading.get_candles_volumes_from_instruments(TEXT[], timestamp, int, int, int, bool) IS 'From a set of currencies, and a timestamp, get the associated forex candles with their bid and ask volumes.';

--TEST
--SELECT * FROM trading.get_candles_volumes_from_instruments(ARRAY['EUR','USD','GBP','JPY'], '07 Mar 2022 12:30:00'::timestamp, 100) 
