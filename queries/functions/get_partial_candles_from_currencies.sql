...
BEGIN 
	...
	DROP TABLE IF EXISTS _get_partial_candles_from_currencies;
	CREATE TEMPORARY TABLE _get_partial_candles_from_currencies
	ON COMMIT DROP 
	AS (
		WITH the_range AS (
			SELECT 
	...
	CREATE INDEX _get_partial_candles_from_currencies_row_index_idx ON _get_partial_candles_from_currencies USING btree(row_index);
	CREATE INDEX _get_partial_candles_from_currencies_full_name_idx ON _get_partial_candles_from_currencies USING btree(full_name);
	CREATE INDEX _get_partial_candles_from_currencies_the_date_idx ON _get_partial_candles_from_currencies USING btree(the_date); --check

	RETURN QUERY SELECT * FROM _get_partial_candles_from_currencies; 
