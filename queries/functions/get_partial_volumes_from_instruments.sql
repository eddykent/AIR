...
DROP FUNCTION IF EXISTS trading.get_partial_volume_from_instruments(TEXT[], TIMESTAMP[], int, int);
CREATE OR REPLACE FUNCTION trading.get_partial_volumes_from_instruments(_instruments TEXT[], _trade_times TIMESTAMP[], _chart_resolution INTEGER DEFAULT 15, _candle_offset INTEGER DEFAULT 0)
RETURNS TABLE (
	row_index INTEGER,	
	from_currency TEXT,
	to_currency TEXT,
...
BEGIN 
	...
	DROP TABLE IF EXISTS _get_partial_volumes_from_instruments;
	CREATE TEMPORARY TABLE _get_partial_volumes_from_instruments
	ON COMMIT DROP 
	AS (
		WITH the_range AS (
			SELECT 
	...
	CREATE INDEX _get_partial_volumes_from_instruments_row_index_idx ON _get_partial_volumes_from_instruments USING btree(row_index);
	CREATE INDEX _get_partial_volumes_from_instruments_full_name_idx ON _get_partial_volumes_from_instruments USING btree(full_name);
	CREATE INDEX _get_partial_volumes_from_instruments_the_date_idx ON _get_partial_volumes_from_instruments USING btree(the_date); --check

	RETURN QUERY SELECT * FROM _get_partial_volumes_from_instruments; 

END
$$ LANGUAGE plpgsql;

COMMENT ON FUNCTION trading.get_partial_volumes_from_instruments(TEXT[], TIMESTAMP[], int, int) IS 'From instrument names and candle end times, get the associated forex partial volumes';
