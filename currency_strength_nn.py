...
the_date = datetime.datetime(2022,2,1,8,0) #hard
#the_date = datetime.datetime(2022,2,3,8,0) #easy 
...
params = {
	...
	'ema_window':5 #average over 4 candles
}
...
sequence_len = 35 #almost 6 days of data
tz = TimeZipper()
tz.x_sequence_lengths = [sequence_len,sequence_len]
tz.number_of_sequences = 1000  + 1 #add one for the step 
...
sap_cs.features = ['rank','average_rank']
...
##point in this step is to stablise the network before training with the correct loss 
model_history = model.fit([train_x1,train_x2],train_y,
	callbacks=[EarlyStoppingAfter(start_epoch=200,monitor='loss', patience=10)],
	batch_size=len(train_x1),
	shuffle=True,
	epochs=500,
	validation_data=([test_x1,test_x2],test_y)
	#validation_split=0.01
)  ##perhaps use validation_split=0.01 and use an early stopping criteria 

#K.set_value(model.optimizer.learning_rate, 0.001)


y_hat = model.predict([test_x1,test_x2])

sorted_ranks = sorted(y_hat[0])
predicted_ranks1 = np.array([sorted_ranks.index(x)+1 for x in y_hat[0]],dtype=int)

sorted_orig = sorted(test_y[0])
actual_ranks1 = np.array([sorted_orig.index(x)+1 for x in test_y[0]],dtype=int)

loss_str1 = 'Loss was: ' + str(ranking_loss(y_hat,test_y))

#print(predicted_ranks1)
#print(actual_ranks1)
#print('Loss was: ' + loss_str1)



weights = model.get_weights() 
model.compile(loss=ranking_loss,optimizer=keras.optimizers.Adam(learning_rate=0.0001))
model.set_weights(weights) #warm start - should be stablised 


#lets only train on last 2/5 samples? 
#last_few = (len(train_x1) * 7) // 8
last_few = 0
model_history = model.fit([train_x1[last_few:],train_x2[last_few:]],train_y[last_few:],
	callbacks=[EarlyStoppingAfter(start_epoch=10,monitor='loss', patience=10)],
	shuffle=True,
	batch_size=10,
	epochs=200,
	validation_data=([test_x1,test_x2],test_y))
...
sorted_ranks = sorted(y_hat[0])
predicted_ranks = np.array([sorted_ranks.index(x)+1 for x in y_hat[0]],dtype=int)
...
print(actual_ranks)
print('Loss was: ' + str(ranking_loss(y_hat,test_y)))
